...
namespace argent::graphics
{
	MeshVertexBuffer::MeshVertexBuffer(ID3D12Device* device, const MeshData& mesh_data)
	{
		...
		//bone_index_buffer_ = std::make_unique<VertexBuffer<uint32_t[4]>>(device, mesh_data.bone_index_);
		//bone_weight_buffer = std::make_unique<VertexBuffer<float[4]>>(device, mesh_data.bone_weights_);
		//vertex_buffer_ = std::make_unique<VertexBuffer<Vertex>>(device, mesh_data.vertices_);
	}
	...
	//Mesh::Mesh(ID3D12Device* device, const MeshData& mesh_data):
	...
	//	auto descriptor_heap = graphics->GetDescriptorHeap(dx12::DescriptorHeap::HeapType::CbvSrvUav);
	//	position_structured_buffer_ = std::make_unique<StructuredBuffer>(device, graphics->GetCommandQueue(),
	...
	//	normal_structured_buffer_ = std::make_unique<StructuredBuffer>(device, graphics->GetCommandQueue(),
	//		mesh_data.normal_.size(), sizeof(DirectX::XMFLOAT3), mesh_data.normal_.data(),
	//		descriptor_heap->PopDescriptor(), L"Vertex Structured Buffer");

	//	tangent_structured_buffer_ = std::make_unique<StructuredBuffer>(device, graphics->GetCommandQueue(),
	//		mesh_data.tangent_.size(), sizeof(DirectX::XMFLOAT3), mesh_data.tangent_.data(),
	//		descriptor_heap->PopDescriptor(), L"Vertex Structured Buffer");

	//	binormal_structured_buffer_ = std::make_unique<StructuredBuffer>(device, graphics->GetCommandQueue(),
	//		mesh_data.binormal_.size(), sizeof(DirectX::XMFLOAT3), mesh_data.binormal_.data(),
	//		descriptor_heap->PopDescriptor(), L"Vertex Structured Buffer");

	//	texcoord_structured_buffer_ = std::make_unique<StructuredBuffer>(device, graphics->GetCommandQueue(),
	//		mesh_data.texcoord_.size(), sizeof(DirectX::XMFLOAT2), mesh_data.texcoord_.data(),
	//		descriptor_heap->PopDescriptor(), L"Vertex Structured Buffer");

	//	bone_structured_buffer_ = std::make_unique<StructuredBuffer>(device, graphics->GetCommandQueue(),
	//		mesh_data.bone_vertices_.size(), sizeof(BoneVertex), mesh_data.bone_vertices_.data(), 
	//		descriptor_heap->PopDescriptor(), L"Vertex Structured Buffer");

	//	vertex_index_structured_buffer_ = std::make_unique<StructuredBuffer>(device, graphics->GetCommandQueue(),
	//		mesh_data.vertex_index_.size(), sizeof(uint32_t), mesh_data.vertex_index_.data(), graphics->GetDescriptorHeap(dx12::DescriptorHeap::HeapType::CbvSrvUav)->PopDescriptor(),
	//		L"Vertex Index Structured Buffer");

	//	meshlet_structured_buffer_ = std::make_unique<StructuredBuffer>(device, graphics->GetCommandQueue(),
	//		mesh_data.meshlets_.size(), sizeof(Meshlet), mesh_data.meshlets_.data(), graphics->GetDescriptorHeap(dx12::DescriptorHeap::HeapType::CbvSrvUav)->PopDescriptor(),
	//		 L"Meshlet Structured Buffer");

	//	primitive_structured_buffer_ = std::make_unique<StructuredBuffer>(device, graphics->GetCommandQueue(),
	//		mesh_data.primitive_index_.size(), sizeof(PackedPrimitiveData), mesh_data.primitive_index_.data(), graphics->GetDescriptorHeap(dx12::DescriptorHeap::HeapType::CbvSrvUav)->PopDescriptor()
