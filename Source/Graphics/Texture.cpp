...
{
	//Texture::Texture(ID3D12Device* device, ID3D12CommandQueue* command_queue, const wchar_t* filename,
	//	const dx12::Descriptor& srv_descriptor):
	//srv_descriptor_(srv_descriptor)
	//{
	//	LoadTexture(device, command_queue, filename);
	...
	//	device->CreateShaderResourceView(texture_resource_.Get(), &shader_resource_view_desc, srv_descriptor_.GetCpuHandle());

	//	width_ = static_cast<float>(texture_resource_->GetDesc().Width);
	//	height_ = static_cast<float>(texture_resource_->GetDesc().Height);
	//}

	//Texture::Texture(ID3D12Device* device, ID3D12CommandQueue* command_queue, const char* filename,
	//	const dx12::Descriptor& srv_descriptor):
	//srv_descriptor_(srv_descriptor)
	//{
	//	std::filesystem::path filepath = filename;
	//	LoadTexture(device, command_queue, filepath.wstring().c_str());
